...
use crate::app_state::{AppError, ErrorOutcome, JobEvent, JobId, RunMode};
...
fn execute(
    ...
    match request.kind {
        ...
        WorkerRequestKind::Discover { inputs } => JobEvent::DiscoveryFinished {
            job_id,
            result: discover_catalog_paths(&inputs)
                .map_err(|error| AppError::no_data_change(error.to_string())),
        },
...
fn map_browser_error(error: BrowserError) -> AppError {
    match error {
        BrowserError::LoginRequired => AppError::login_required("ChatGPT 로그인 필요"),
        BrowserError::LoginBrowserStillOpen => AppError::login_browser_still_open(),
        BrowserError::TargetChanged
        | BrowserError::NoChatGptTarget
        | BrowserError::AmbiguousTarget => AppError::browser_reconnect_required(error.to_string()),
        BrowserError::PageNotReady
        | BrowserError::AuthorizationDenied
        | BrowserError::RateLimited
        | BrowserError::WebUnavailable => AppError::retryable_authentication(error.to_string()),
        _ => AppError::no_data_change(error.to_string()),
    }
}
...
fn collect_web(
    ...
    let session = browser
        .as_mut()
        .ok_or_else(|| AppError::no_data_change("브라우저 로그인을 먼저 열어야 함"))?;
...
fn web_collector_expression(report: &ScanReport) -> Result<String, AppError> {
    ...
    if report.threads.is_empty() || report.threads.len() > 10_000 {
        return Err(AppError::no_data_change(
            "웹 대조 대상은 1~10,000개 범위여야 함",
        ));
    }
    ...
    let ids =
        serde_json::to_string(&ids).map_err(|_| AppError::no_data_change("웹 요청 생성 실패"))?;
    let groups = serde_json::to_string(&groups)
        .map_err(|_| AppError::no_data_change("웹 요청 생성 실패"))?;
    Ok(format!(
...
fn map_collector_error(error: &str) -> AppError {
    ...
    let message = match error {
        "authorization_failed" => {
            "웹 접근 권한을 확인하지 못함. 로그인 상태 확인 후 다시 대조해야 함"
        }
        "account_changed" => "조회 중 계정이 달라졌거나 계정 정보를 검증하지 못함",
        "session_user_mismatch" => "로그인 세션과 인증 사용자 정보가 일치하지 않음",
        "account_match_mismatch" => "로그인 계정과 일치하는 계정을 확정하지 못함",
        "account_match_none" => "로그인 계정에 맞는 계정 항목이 없음",
        "account_match_multiple" => "로그인 계정에 맞는 계정 항목이 여러 개임",
        "ordered_personal_account_id_missing" => "개인 계정 항목에 계정 ID가 없음",
        "ordered_personal_account_id_missing_bound" => "개인 계정 ID 없음 · 로그인 계정 일치",
        "ordered_personal_account_id_missing_unbound" => "개인 계정 ID 없음 · 로그인 계정 미확인",
        "account_user_mismatch" => "계정과 로그인 사용자 연결이 일치하지 않음",
        "final_identity_changed" => "확인 중 로그인 계정이 바뀜",
        "unsupported_account" => "현재 버전은 개인 계정만 지원. Workspace 계정은 정리 차단",
        "rate_limited" => "웹 조회 횟수 제한에 도달함. 잠시 후 다시 시도해야 함",
        "collection_timeout" => "웹 확인 시간이 초과됨. 다시 시도해야 함",
        "no_positive_control" => {
            "이 계정에서 확인 가능한 로컬 대화를 찾지 못해 대조 기준을 확립하지 못함"
        }
        "inventory_changed" => "조회 중 웹 대화 목록이 바뀜",
        "inventory_total_changed" => "조회 중 대화 총개수가 바뀜",
        "inventory_duplicate" => "같은 목록에서 대화가 중복됨",
        "inventory_partition_overlap" => "보관된 대화와 일반 대화 목록이 겹침",
        "inventory_count_overflow" => "받은 대화 수가 조회 범위를 초과함",
        "inventory_invalid_pagination" => "다음 대화 목록 위치가 올바르지 않음",
        "positive_control_missing" => "확인했던 대화를 다시 조회하지 못함",
        "inventory_head_changed" => "재확인한 대화 목록의 처음 부분이 바뀜",
        "incomplete_inventory" => "전체 개수보다 적은 대화 목록을 받음",
        "limit_exceeded" => "대화 목록 조회 횟수 한도를 초과함",
        "invalid_request" => "로컬 대화 ID 형식 또는 개수가 웹 조회 조건과 맞지 않음",
        _ => "웹 조회를 완료하지 못함. 로그인 상태·네트워크·웹 응답 형식을 확인해야 함",
    };
    AppError::no_data_change(message)
}
...
fn map_repair_error(error: RepairError) -> AppError {
    let outcome = match &error {
        ...
        | RepairError::CommitOutcomeInvalid { .. } => ErrorOutcome::OutcomeUncertain,
        _ => ErrorOutcome::NoDataChange,
    };
    AppError {
...
mod tests {
    ...
    use super::*;
    use crate::app_state::{AppState, Operation};
